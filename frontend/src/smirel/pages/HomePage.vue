    ...
    <main class="home-content">
      ...
      <section class="home-grid two-col">
        <article class="glass story-card">
          <span class="eyebrow">GET STARTED</span><h2>三步接入。</h2><p>创建账户、生成 Key、替换 Base URL。其余调用方式保持不变。</p>
          <RouterLink :to="isAuthenticated ? '/keys' : '/register'">开始使用 →</RouterLink>
        </article>
      ...
      <section class="home-grid two-col routing-row">
        <article class="glass story-card"><span class="eyebrow">MODEL ROUTING</span><h2>模型可以换，接入地址不用换。</h2><p>OpenAI、Anthropic、Google 使用统一入口。模型能力和价格在一个地方管理。</p><div class="provider-line"><span>OpenAI</span><span>Anthropic</span><span>Google</span></div></article>
        <article class="glass code-card"><header><span>OpenAI SDK</span><span>Python</span></header><pre><code>from openai import OpenAI

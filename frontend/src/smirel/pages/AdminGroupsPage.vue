...
type DisplayIcon = 'auto' | 'claude' | 'openai' | 'gemini' | 'grok' | 'antigravity' | 'generic'
type ResolvedDisplayIcon = Exclude<DisplayIcon, 'auto'>
type ModelsListConfig = { enabled?: boolean; models?: string[]; display_icon?: DisplayIcon }
type GroupRow = {
...
const form = reactive({ name: '', description: '', platform: 'openai', rate_multiplier: 1, subscription_type: 'standard', status: 'active', display_icon: 'auto' as DisplayIcon })
...
function modelsConfig(g: GroupRow): ModelsListConfig { return g.models_list_config || {} }
function configuredModels(g: GroupRow) { return Array.isArray(modelsConfig(g).models) ? modelsConfig(g).models! : [] }
function normalizeDisplayIcon(value: unknown): DisplayIcon {
  return ['auto', 'claude', 'openai', 'gemini', 'grok', 'antigravity', 'generic'].includes(String(value)) ? String(value) as DisplayIcon : 'auto'
}
function fallbackIconForPlatform(v: string): ResolvedDisplayIcon {
  return ({ openai: 'openai', anthropic: 'claude', gemini: 'gemini', antigravity: 'antigravity', grok: 'grok', composite: 'generic' } as Record<string, ResolvedDisplayIcon>)[v] || 'generic'
}
...
function resolvedGroupIcon(g: GroupRow): ResolvedDisplayIcon {
  const configured = normalizeDisplayIcon(modelsConfig(g).display_icon)
  if (configured !== 'auto') return configured
  const models = configuredModels(g).map(model => model.trim()).filter(Boolean)
...
function openCreate() {
  ...
  Object.assign(form, { name: '', description: '', platform: 'openai', rate_multiplier: 1, subscription_type: 'standard', status: 'active', display_icon: 'auto' as DisplayIcon })
  showEditor.value = true
}
function openEdit(g: GroupRow) {
  editingId.value = g.id
  Object.assign(form, { name: g.name, description: g.description || '', platform: g.platform, rate_multiplier: Number(g.rate_multiplier || 1), subscription_type: g.subscription_type || 'standard', status: g.status || 'active', display_icon: normalizeDisplayIcon(modelsConfig(g).display_icon) })
  showEditor.value = true
}
async function saveGroup() {
  ...
  try {
    ...
    if (editingId.value) {
      const currentGroup = groups.value.find(g => g.id === editingId.value)
      await api.put(`/admin/groups/${editingId.value}`, {
        ...payload,
        models_list_config: {
...
async function saveModelWhitelist(g: GroupRow) {
  ...
  try {
    await api.put(`/admin/groups/${g.id}`, { models_list_config: { ...modelsConfig(g), enabled: true, models } })
      ...
      <div class="group-list">
        <article v-for="g in visibleGroups" :key="g.id" class="group-card" :class="{ open: expandedId === g.id }">
          <button class="group-main" :aria-expanded="expandedId === g.id" @click="toggleGroup(g)">
            <span class="mark group-provider-mark" :data-icon="resolvedGroupIcon(g)">{{ displayIconMark(resolvedGroupIcon(g)) }}</span>
    ...
    <div v-if="showEditor" class="overlay" @click.self="showEditor = false">
      <form class="dialog" @submit.prevent="saveGroup">
        ...
        <div class="form-grid">
          ...
          <label v-if="editingId" class="display-icon-field"><span>展示图标</span><UiSelect v-model="form.display_icon" :options="displayIconOptions" aria-label="展示图标" fluid /><small class="display-icon-help">仅影响界面展示，不改变接口协议、账号匹配和调度行为。</small></label>

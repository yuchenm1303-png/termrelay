...
const displayName = computed(() => state.user?.username || state.user?.email?.split('@')[0] || 'Muxway Account')
const secondaryText = computed(() => state.user?.email || (isAdmin.value ? '管理员账户' : 'Muxway 账户'))
